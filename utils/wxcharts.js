...
function calCategoriesData(categories, opts, config) {
    ...
    //     console.log(result.xAxisHeight)
    // }
...
function getXAxisPoints(categories, opts, config) {
    ...
    var spacingValid = opts.width - 3*config.padding - yAxisTotalWidth;
    // var dataCount = opts.enableScroll ? Math.min(5, categories.length) : categories.length;
    var dataCount = categories.length;
...
function getYAxisTextList(series, opts, config) {
  ...
  //  var minData = Math.min.apply(this, data);
    ...
    var range = [];
    var eachRange = (maxRange - minRange) / config.yAxisSplit+0.25;
    ...
    if (shape === 'diamond') {
        points.forEach(function (item, index) {
          if (item !== null) {
            ...
            context.drawImage('../../../images/data_point@3x.png', item.x-5 , item.y-6 , 10, 10)
            context.stroke();
            context.closePath();
...
function drawPointText(points, series, config, context, eachSpacing) {
    // 绘制数据文案
    ...
    points.forEach(function (item, index) {
        if (item !== null) {
            ...
            var spacingValid = screenWidth - 3 * config.padding - config.yAxisWidth + config.yAxisTitleWidth;
            
            var space = spacingValid / 4 - item.width

            console.log(spacingValid/4, eachSpacing)
             if (screenWidth <330){
            
              context.drawImage('../../../images/tooltip-拷贝@3x.png', item.x-1 , item.y - 40, space+2 , 30)
              context.setTextAlign('center')
              context.fillText(formatVal, item.x + space / 2, item.y - 20);
            }else{
               context.drawImage('../../../images/tooltip-拷贝@3x.png', item.x, item.y - 40, space -2, 30)
               context.setTextAlign('center')
               context.fillText(formatVal, item.x + space / 2, item.y - 20);
            }
...
function drawToolTip(textList, offset, opts, config, context) {
  console.log(textList)
    ...
    var toolTipWidth = legendWidth + legendMarginRight + 4 * config.toolTipPadding + Math.max.apply(null, textWidth)+20;
...
//context.drawImage('../../../images/data_tooltip@3x.png', offset.x - 38, offset.y - 30, 73, 35)
  ...
  textList.forEach(function (item, index) {
    ...
    context.drawImage('../../../images/data_point_present@3x.png', offset.x-6, offset.y+2, 12, 12)
    context.stroke();
    context.closePath();
...
function drawColumnDataPoints(series, opts, config, context) {
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        points.forEach(function (item, index) {
         ...
         //   var startX = item.x - item.width / 2 + 1;
            var startX = item.x - item.width+1
        ...
        context.setFillStyle('rgb(34,185,110)');
        points.forEach(function (item, index) {
            if (item !== null) {
               // var startX = item.x - item.width / 2 + 1;
              var startX = item.x - item.width+1
                ...
                context.moveTo(startX, item.y);
                context.rect(startX, item.y, item.width - 2, height);
              //   var grd = context.createLinearGradient(item.width, item.y , item.width  ,  150)
                
              //    grd.addColorStop(0, 'rgb(34,185,110)')
              //    grd.addColorStop(1, 'rgb(28,88,235)')
              // context.setFillStyle(grd)
              //  context.fillRect(startX, item.y, item.width - 2, height, index)

                context.drawImage('../../../images/图层-18@3x.png', startX, item.y-16, 16, 16)
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        var points = getDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts, config, process);
        points = fixColumeData(points, eachSpacing, series.length, seriesIndex, config, opts);
        if (opts.dataLabel !== false && process === 1) {        //数值
          drawPointText(points, eachSeries, config, context, eachSpacing);
        }
    });
    context.restore();
    return {
...
function drawXAxis(categories, opts, config, context) {
    ...
    // if (config._xAxisTextAngle_ === 0) {
        ...
        categories.forEach(function (item, index) {
            ...
            if (opts.type == 'area'){
              ...
              if(offset>20){
                context.setTextAlign('center')
                context.fillText(item, xAxisPoints[index] + offset+2, startY + config.fontSize + 5);
              }else{
   ...
   // } 
    ...
    //         context.fillText(item, xAxisPoints[index] + offset, startY + config.fontSize + 5);
    //         context.closePath();
        config.xAxisHeight    //         context.restore();
    //     });
    // }
...
function drawYAxisGrid(opts, config, context,) {
    ...
    console.log( config.xAxisHeight)
    ...
    points.forEach(function (item, index) {
     ...
     if(index!=4){
       var dashLen = 1 === undefined ? 3 : 1,
         xpos = endX - startX, //得到横向的宽度;
         ypos = 0, //得到纵向的高度;
         numDashes = Math.floor(Math.sqrt(xpos * xpos + ypos * ypos) / dashLen);
       //利用正切获取斜边的长度除以虚线长度，得到要分为多少段;
       for (var i = 0; i < numDashes; i++) {
         if (i % 2 === 0) {
           context.moveTo(startX + (xpos / numDashes) * i, item + (ypos / numDashes) * i);
           //有了横向宽度和多少段，得出每一段是多长，起点 + 每段长度 * i = 要绘制的起点；
         } else {
           context.lineTo(startX + (xpos / numDashes) * i, item + (ypos / numDashes) * i);
         }
       }
     }else{
...
function drawYAxis(series, opts, config, context) {
    ...
    var yAxisTotalWidth = config.yAxisWidth + config.yAxisTitleWidth;
    //var yAxisTotalWidth=0
    //console.log(yAxisTotalWidth)

    var spacingValid = opts.height - 2 * config.padding - config.xAxisHeight - config.legendHeight;
    
    var eachSpacing = Math.floor(spacingValid / config.yAxisSplit) ;
    var startX = config.padding + yAxisTotalWidth;
    var endX = opts.width - config.padding+20;
    var endY = opts.height - config.padding - config.xAxisHeight - config.legendHeight;
    ...
    rangesFormat.forEach(function (item, index) {
       ...
       // Y轴数值
        // if (opts.type == 'column'){
        //   if (item > 10) {
        //     context.fillText(item, config.padding + config.yAxisTitleWidth , pos + config.fontSize / 4);
        //   } else {
...
Charts.prototype.showToolTip = function (index) {
